...
const MainnetStats = () => {
    ...
    useEffect(() => {
        ...
        fetchBtc();
        const priceInterval = setInterval(fetchBtc, 10000);
    ...
    return (
        <div className="container mx-auto px-2 sm:px-4 my-4 sm:my-6">
            <div className="relative w-full bg-card/60 rounded-lg border border-primary/30 glow-border-primary overflow-hidden backdrop-blur-sm">
                <div className="flex justify-between items-center divide-x divide-primary/20">
                    <StatIndicator label="BTC Price" value={btcPrice ? `$${parseFloat(btcPrice).toLocaleString()}` : <Loader2 className="h-4 w-4 animate-spin"/>} valueClassName="text-white" />
