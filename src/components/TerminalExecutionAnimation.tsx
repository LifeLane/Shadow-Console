...
const TerminalExecutionAnimation: React.FC<TerminalExecutionAnimationProps> = ({
  ...
  return (
    <Card className="bg-black/80 p-4 rounded-lg shadow-2xl font-code text-sm h-auto min-h-[300px] max-h-[450px] overflow-y-auto border border-primary/50 glow-border-primary">
      <CardContent className="p-0">
        {logLines.map((line, index) => {
          ...
          return (
            <div key={line.id} className={cn("mb-1 min-h-[1.5em]", line.color || "text-primary")}>
              <TypewriterText
                text={line.text}
                speed={line.speed}
                ...
                caretClassName="bg-primary animate-blink-block-caret opacity-100"
              />
            </div>
          );
        })}
